...
// 模拟生成全国气象站点分钟观测数据，存放在cscurfdata容器中。
void CrtSurfData();
...
int main (int argc,char *argv[]) {
  ...
  if ( (argc != 5) && (argc != 6) ) {
    ...
    printf("        /project/idc1/bin/crtsurfdata /project/idc1/ini/stcode.ini /tmp/idc/surfdata /log/idc/crtsurfdata.log xml,json,csv 20220217145300\n\n");
  ...
  // 模拟生成全国气象站点分钟观测数据，存放在vsyrfdata容器中。
  CrtSurfData();
...
//模拟生成全国气象站点分钟观测数据，存放在vssurfdata容器中。
void CrtSurfData() {
  // 播随机种子
  srand(time(0));
